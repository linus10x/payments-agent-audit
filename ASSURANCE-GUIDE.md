...
- **Unit + contract** (`tests/test_*.py`) — every primitive, control, and
  obligation entry has direct tests; public APIs are pinned. The current build is
  183 tests at 98.97% coverage.
- **Property-based** (`tests/property/`) — up to ~2,300 generated cases per run (a
  ceiling set by Hypothesis `max_examples`, not a floor) across ledger
  append/verify/tamper invariants, the irreversibility algebra, DEFCON
  transition-direction algebra, veto un-self-clearability, and OFAC exact-match.
- **Golden corpus** (`tests/golden/`) — 10 real, primary-sourced enforcement /
  litigation / incident / guidance matters turned into parametrized fixtures asserting how
...
- **Adversarial** (`tests/adversarial/`) — the six AL-PROBES across 11 committed
  test functions (AL-PROBE-03 and AL-PROBE-06 each carry sub-tests).
- **Mutation** (`scripts/mutation_pass.py`) — a surgical pass over the
  load-bearing invariants; the current pass kills all 23 targeted mutants (100%
  kill rate). A surviving mutant means an assertion is too weak.
...
| Reg E error resolution | 12 CFR 1005.11 (10/45-day standard; 20/90-day new-account/POS/foreign) |
...
| Effective challenge | Model-risk-management "effective challenge" (origin: SR 11-7 / OCC 2011-12; carried into subsequent MRM-guidance revisions) |
